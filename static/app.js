...
const CUSTOM_TEXT = 'Другое (ввести вручную)';
...
async function loadMakes() {
  ...
  for (const m of makes) { const o=document.createElement('option'); o.value=m; o.textContent=m; els.make.appendChild(o); }
  const custom = document.createElement('option'); custom.value='__custom__'; custom.textContent=CUSTOM_TEXT; els.make.appendChild(custom);
}

async function loadModels(make) {
  els.model.disabled = true; els.model.innerHTML='';
  els.generationRow.style.display='none'; els.generation.innerHTML='';
  if (!make || make==='__custom__') {
    els.model.disabled=false;
    const custom=document.createElement('option'); custom.value='__custom__'; custom.textContent=CUSTOM_TEXT; custom.selected=true; els.model.appendChild(custom);
    if (els.modelCustomWrap) els.modelCustomWrap.style.display='';
    els.generationRow.style.display=''; const go=document.createElement('option'); go.value='__custom__'; go.textContent=CUSTOM_TEXT; go.selected=true; els.generation.appendChild(go);
    if (els.genCustomWrap) els.genCustomWrap.style.display='';
    return;
  }
  const models = await fetchJSON(`/api/models?make=${encodeURIComponent(make)}`);
  const def=document.createElement('option'); def.value=''; def.textContent='— выберите модель —'; def.disabled=true; def.selected=true; els.model.appendChild(def);
  for (const m of models) { const o=document.createElement('option'); o.value=m; o.textContent=m; els.model.appendChild(o); }
  const custom=document.createElement('option'); custom.value='__custom__'; custom.textContent=CUSTOM_TEXT; els.model.appendChild(custom);
  els.model.disabled=false; if (els.modelCustomWrap) els.modelCustomWrap.style.display='none'; if (els.genCustomWrap) els.genCustomWrap.style.display='none';
}

async function loadGenerations(make, model) {
  els.generationRow.style.display='none'; els.generation.innerHTML='';
  if (!make || !model || make==='__custom__' || model==='__custom__') {
    els.generationRow.style.display=''; const co=document.createElement('option'); co.value='__custom__'; co.textContent=CUSTOM_TEXT; co.selected=true; els.generation.appendChild(co); if (els.genCustomWrap) els.genCustomWrap.style.display=''; return null;
  }
  const gens = await fetchJSON(`/api/generations?make=${encodeURIComponent(make)}&model=${encodeURIComponent(model)}`);
  const def=document.createElement('option'); def.value=''; def.textContent='— выберите поколение —'; def.disabled=true; def.selected=true; els.generation.appendChild(def);
  for (const g of gens) { const o=document.createElement('option'); o.value=String(g.id); o.textContent=g.label || `generation ${g.id}`; els.generation.appendChild(o); }
  const custom=document.createElement('option'); custom.value='__custom__'; custom.textContent=CUSTOM_TEXT; els.generation.appendChild(custom);
...
// Events
els.make.addEventListener('change', async()=>{ const isCustom=els.make.value==='__custom__'; if(els.makeCustomWrap) els.makeCustomWrap.style.display=isCustom?'':'none'; await loadModels(els.make.value); });
els.model.addEventListener('change', async()=>{ const isCustom=els.model.value==='__custom__'; if(els.modelCustomWrap) els.modelCustomWrap.style.display=isCustom?'':'none'; await loadGenerations(els.make.value, els.model.value); });
if(els.generation){ els.generation.addEventListener('change',()=>{ const isCustom=els.generation.value==='__custom__'; if(els.genCustomWrap) els.genCustomWrap.style.display=isCustom?'':'none'; const gid=currentVehicleId(); if(els.drawer && els.drawer.classList.contains('open')){ if(gid) loadExistingParams(gid); else { if(els.existingList) els.existingList.innerHTML=''; if(els.existingInfo) els.existingInfo.textContent='Кастомное ТС или поколение не выбрано.'; } } }); }

// Submit
els.form.addEventListener('submit', async (e)=>{
  e.preventDefault(); setStatus('Отправка...', true);
  try{
    const vehicleId=currentVehicleId(); let generationLabel=null; if(els.generation && els.generation.selectedOptions && els.generation.selectedOptions.length){ generationLabel=els.generation.selectedOptions[0].textContent; }
    const payload={ vehicle_id: vehicleId, make: els.make.value==='__custom__'? null: els.make.value, make_custom: els.makeCustomInput? (els.makeCustomInput.value||null): null, model: els.model.value==='__custom__'? null: els.model.value, model_custom: els.modelCustomInput? (els.modelCustomInput.value||null): null, generation_label: generationLabel, generation_custom: els.genCustomInput? (els.genCustomInput.value||null): null, items: paramItems.map(it=>it.toJSON()) };
...
function addParamItem(){
  const idx=paramItems.length+1; const node=document.createElement('div'); node.className='param-item';
  node.innerHTML=`
    ...
    <div class="param-grid three">
      <div class="field">
        ...
        <div class="row param-custom-wrap" style="display:none; margin-top:6px;">
...
function makeParamItem(node){
  ...
  const customWrap=node.querySelector('.param-custom-wrap');
  ...
  const busType=node.querySelector('.bus-type');
  const busSpeed=node.querySelector('.bus-speed');
  const dimension=node.querySelector('.dimension');
  const bitgrid=node.querySelector('.bitgrid');
  const removeBtn=node.querySelector('.remove-param');
  const offsetBits=node.querySelector('.offset-bits');
  const lengthBits=node.querySelector('.length-bits');
  const is29bit=node.querySelector('.is29bit');
